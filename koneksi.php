...
function ensureSchemaUpdates(PDO $pdo): void
{
    ...
    ensureCompatibilityViews($pdo);
}

function ensureCompatibilityViews(PDO $pdo): void
{
    try {
        $pdo->exec('CREATE OR REPLACE VIEW interaksi AS SELECT * FROM aturan_interaksi');
    } catch (PDOException $e) {
        // The application uses aturan_interaksi directly. This legacy alias is best-effort only.
    }
}
