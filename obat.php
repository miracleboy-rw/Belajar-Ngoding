...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nama !== '') {
        if ($id > 0) {
            $stmt = $pdo->prepare('UPDATE obat SET nama_obat = ?, kategori = ? WHERE id = ?');
            $stmt->execute([$nama, $kategori, $id]);
        } else {
            $stmt = $pdo->prepare('INSERT INTO obat (nama_obat, kategori) VALUES (?, ?)');
        ...
        <div class="card">
            ...
            <div class="card-body table-responsive">
                <table class="table table-hover align-middle">
                    <thead><tr><th>Nama Obat</th><th>Kategori</th><th>Aksi</th></tr></thead>
                    <tbody>
