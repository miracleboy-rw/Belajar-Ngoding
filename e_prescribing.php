...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($pasienId > 0 && $details) {
        ...
        $stmt = $pdo->prepare('INSERT INTO resep (pasien_id, tanggal) VALUES (?, NOW())');
        $stmt->execute([$pasienId]);
        $resepId = (int) $pdo->lastInsertId();

        $stmtDetail = $pdo->prepare('INSERT INTO detail_resep (resep_id, obat_id, dosis, aturan_pakai) VALUES (?, ?, ?, ?)');
        foreach ($details as $detail) {
            $stmtDetail->execute([$resepId, $detail['obatId'], $detail['dosis'], $detail['aturan']]);
        }
        $pdo->commit();
        $successMessage = 'Resep berhasil disimpan dengan nomor #' . $resepId . '.';
    } else {
        ...
        <td>
            <select name="obat_id[]" class="form-select obat-select" required>
                ...
                <?php foreach ($obatList as $obat): ?>
                    <option value="<?= e($obat['id']); ?>"><?= e($obat['nama_obat']); ?> - <?= e($obat['kategori']); ?></option>
                <?php endforeach; ?>
            </select>
        </td>
